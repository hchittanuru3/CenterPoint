...
def label_to_tensor(track_group_dict: Dict, n_t: int) -> Tuple[Dict, List]:
    ...
    for timestamp, tracks in track_group_dict.items():
        ...
        tensors[timestamp] = torch.zeros((num_tracks, n_t, 8))  # 8 values per bounding box
...
def interpolate_track(track: Dict, n_t: int) -> Tuple[torch.tensor, List]:
    ...
    interpolated = torch.zeros((n_t, 8))  # 8 values per bounding box
    ...
    interpolated[to_interpolate, 7] = first_label.label_num
...
def convert_track_to_tensor(bboxes, track):
    ...
    labels = torch.tensor(
        [label.label_num for _, label in track.items()], dtype=torch.float32
    )
    return torch.cat((
